# **Termo de ações — arquivo de instrumentos (BVBG.028.02)**

Leitura do bloco **`EqtyFwdInf`** do arquivo de instrumentos do pregão (`IN{aammdd}.zip`),
publicado em `www.b3.com.br/pesquisapregao/download`. Registra os contratos a **termo de ações**, com o instrumento subjacente e o prazo.
...
`InstrumentsFileEqtyFwdReader` baixa o **mesmo** `IN{aammdd}.zip` que os demais _readers_ da família, mantém apenas
os registros que carregam o bloco `EqtyFwdInf` e mapeia a lista **completa** de campos desse
bloco — **18** campos próprios, mais as colunas comuns a todo instrumento (data de
referência, identificação e atributos comuns), totalizando **31** colunas de origem.

Na sessão reconciliada (`IN260729`), o bloco trouxe **676** registros.
...
O contrato exige `ASST`, `ASST_DESC`, `DSTRBTN_ID`, `MKT_IDR_CD`, `MKT_NM`, `OTHR_ID`, `PMT_TP`, `PRIC_FCTR`, `RPT_DT`, `SCTY_CTGY_NM`, `SGMT_NM`, `TCKR_SYMB`, `TRADG_CCY`, `UNDRLYG_INSTRM_ID`, `UNDRLYG_INSTRM_ID_MKT_IDR_CD`, `UNDRLYG_INSTRM_ID_TP`. Os campos `[0..1]` do bloco fluem como colunas tipadas sem serem
...
from filings_b3.search_trading_session import InstrumentsFileEqtyFwdReader

df = InstrumentsFileEqtyFwdReader(date(2026, 7, 29)).read()
print(df.head())
```
...
df = InstrumentsFileEqtyFwdReader(
