"""Data contract — instruments file, ``ExrcEqtsInf`` block.
...
contract covers the records carrying **``ExrcEqtsInf``** — exercícios de opções sobre ações.
...
``IN260729.zip``: the block declares **10** fields, of which **10** carry a value in that session
...
INSTRUMENTS_FILE_EXRC_EQTS = FileContract(
	"Pesquisa por Pregão Instruments File — ExrcEqtsInf",
