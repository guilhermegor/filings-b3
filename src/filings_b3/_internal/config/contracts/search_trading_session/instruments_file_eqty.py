"""Data contract — instruments file, ``EqtyInf`` block.
...
contract covers the records carrying **``EqtyInf``** — ações e demais valores mobiliários.
...
``IN260729.zip``: the block declares **33** fields, of which **33** carry a value in that session
...
INSTRUMENTS_FILE_EQTY = FileContract(
	"Pesquisa por Pregão Instruments File — EqtyInf",
