"""Data contract — instruments file, ``BTCInf`` block.
...
contract covers the records carrying **``BTCInf``** — BTC (empréstimo de ativos).
...
``IN260729.zip``: the block declares **4** fields, of which **4** carry a value in that session
...
INSTRUMENTS_FILE_BTC = FileContract(
	"Pesquisa por Pregão Instruments File — BTCInf",
