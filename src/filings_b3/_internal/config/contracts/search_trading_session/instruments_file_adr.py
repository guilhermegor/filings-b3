"""Data contract — instruments file, ``ADRInf`` block.
...
contract covers the records carrying **``ADRInf``** — ADRs.
...
``IN260729.zip``: the block declares **8** fields, of which **8** carry a value in that session
...
INSTRUMENTS_FILE_ADR = FileContract(
	"Pesquisa por Pregão Instruments File — ADRInf",
