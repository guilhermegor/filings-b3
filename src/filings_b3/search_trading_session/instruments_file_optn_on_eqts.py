"""Pesquisa por Pregão instruments file — ``OptnOnEqtsInf`` projection.
...
exactly one ``<InstrmInf>`` sub-block, so this reader keeps the records carrying ``OptnOnEqtsInf``
and maps that block's **complete** field list (27 fields per B3's taxonomy, 27 of them populated in
the reconciled session): opções sobre ações.
...
from filings_b3._internal.config.contracts import INSTRUMENTS_FILE_OPTN_ON_EQTS
from filings_b3.search_trading_session._base_instruments_file_reader import (
	_BaseInstrumentsFileReader,
)


# Column to path, relative to the <OptnOnEqtsInf> sub-block. Generated from B3's BVBG.028
...
_DICT_PATHS: dict[str, str] = {
	...
	"AUTOMTC_EXRC_IND": "AutomtcExrcInd",
	"CFICD": "CFICd",
...
_LIST_DECIMAL_COLS: tuple[str, ...] = ("EXRC_PRIC",)


class InstrumentsFileOptnOnEqtsReader(_BaseInstrumentsFileReader):
	"""Reader for the ``OptnOnEqtsInf`` records of B3's instruments file.

	Covers opções sobre ações. Downloads ``IN{yymmdd}.zip``, keeps the records carrying
	``OptnOnEqtsInf``, flattens this block's fields plus the record-level common columns,
	validates against ``INSTRUMENTS_FILE_OPTN_ON_EQTS``, applies explicit types, and stamps
	...
	str_source_key = "instruments_file_optn_on_eqts"
	cls_contract = INSTRUMENTS_FILE_OPTN_ON_EQTS
	str_sub_block = "OptnOnEqtsInf"
