"""Pesquisa por Pregão instruments file — ``CshInf`` projection.
...
exactly one ``<InstrmInf>`` sub-block, so this reader keeps the records carrying ``CshInf``
and maps that block's **complete** field list (4 fields per B3's catalog v2.6, 3 of them populated
in the reconciled session): disponível (cash).
...
from filings_b3._internal.config.contracts import INSTRUMENTS_FILE_CSH
from filings_b3.search_trading_session._base_instruments_file_reader import (
	_BaseInstrumentsFileReader,
)


# Column to path, relative to the <CshInf> sub-block. Derived from B3's catalog v2.6 (block anchor
# 4.13), cross-checked against the taxonomy sheet and a real IN file. ISIN is declared [0..1] and
# was absent from the reconciled session — a cash instrument need not carry one — so it is mapped
# but never required.
...
class InstrumentsFileCshReader(_BaseInstrumentsFileReader):
	"""Reader for the ``CshInf`` records of B3's instruments file.

	Covers disponível (cash). Downloads ``IN{yymmdd}.zip``, keeps the records carrying ``CshInf``,
	flattens this block's fields plus the record-level common columns, validates against
	``INSTRUMENTS_FILE_CSH``, applies explicit types, and stamps provenance. No persistence — a
	...
	str_source_key = "instruments_file_csh"
	cls_contract = INSTRUMENTS_FILE_CSH
	str_sub_block = "CshInf"
