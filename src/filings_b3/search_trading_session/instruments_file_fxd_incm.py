"""Pesquisa por Pregão instruments file — ``FxdIncmInf`` projection.
...
exactly one ``<InstrmInf>`` sub-block, so this reader keeps the records carrying ``FxdIncmInf`` and
maps that block's **complete** field list (13 fields per B3's taxonomy, 13 of them populated in the
reconciled session): renda fixa.
...
from filings_b3._internal.config.contracts import INSTRUMENTS_FILE_FXD_INCM
from filings_b3.search_trading_session._base_instruments_file_reader import (
	_BaseInstrumentsFileReader,
)


# Column to path, relative to the <FxdIncmInf> sub-block. Generated from B3's BVBG.028
...
_DICT_PATHS: dict[str, str] = {
	...
	"TCKR_SYMB": "TckrSymb",
	"TP": "UndrlygInstrmId/OthrId/Tp/Prtry",
	"TRADG_CCY": "TradgCcy",
...
class InstrumentsFileFxdIncmReader(_BaseInstrumentsFileReader):
	"""Reader for the ``FxdIncmInf`` records of B3's instruments file.

	Covers renda fixa. Downloads ``IN{yymmdd}.zip``, keeps the records carrying ``FxdIncmInf``,
	flattens this block's fields plus the record-level common columns, validates against
	``INSTRUMENTS_FILE_FXD_INCM``, applies explicit types, and stamps provenance. No
	...
	str_source_key = "instruments_file_fxd_incm"
	cls_contract = INSTRUMENTS_FILE_FXD_INCM
	str_sub_block = "FxdIncmInf"
