"""Pesquisa por Pregão instruments file — ``EqtyFwdInf`` projection.
...
exactly one ``<InstrmInf>`` sub-block, so this reader keeps the records carrying ``EqtyFwdInf`` and
maps that block's **complete** field list (18 fields per B3's taxonomy, 15 of them populated in the
reconciled session): termo de ações.
...
from filings_b3._internal.config.contracts import INSTRUMENTS_FILE_EQTY_FWD
...
class InstrumentsFileEqtyFwdReader(_BaseInstrumentsFileReader):
	"""Reader for the ``EqtyFwdInf`` records of B3's instruments file.

	Covers termo de ações. Downloads ``IN{yymmdd}.zip``, keeps the records carrying
	``EqtyFwdInf``, flattens this block's fields plus the record-level common columns,
	validates against ``INSTRUMENTS_FILE_EQTY_FWD``, applies explicit types, and stamps
	...
	str_source_key = "instruments_file_eqty_fwd"
	cls_contract = INSTRUMENTS_FILE_EQTY_FWD
	str_sub_block = "EqtyFwdInf"
